(ns edifact.main
  "Kotodama WASM entrypoint for the Edifact clean-room actor (L4) — Clojure port.
  ...
  facts (`edifact.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "edifact")
(def tier "L4")
...
(def entity-specs
  [{:entity "TradeItem" :plural "tradeitems" :id-prefix "edifact_tra"
    :fields [:gtin :description :brand]              :required [:gtin :description]
    :coerce {}                       :refs {}}
   {:entity "Event"     :plural "events"     :id-prefix "edifact_eve"
    :fields [:itemId :bizStep :disposition :recordedAt] :required [:bizStep :disposition]
    :coerce {}                       :refs {}}
   {:entity "Document"  :plural "documents"  :id-prefix "edifact_doc"
    :fields [:type :senderId :receiverId :status]   :required [:type :status]
    :coerce {}                       :refs {}}
   {:entity "Container" :plural "containers" :id-prefix "edifact_con"
    :fields [:bicCode :type :status]                 :required [:bicCode :type]
    :coerce {}                       :refs {}}
   {:entity "Party"     :plural "parties"    :id-prefix "edifact_par"
    :fields [:gln :name :role]                       :required [:gln :name]
    :coerce {}                       :refs {}}
   {:entity "Shipment"  :plural "shipments"  :id-prefix "edifact_shi"
...
(defn emit-facts
  "EAVT facts for one record: {\"edifact.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "edifact-compat" :tier tier :entities entities} 200])
...
(defn start! [] :edifact-compat/ready)
